...
import MyTreeViewModule from "./treeViewModule";
import MyDialogView from "./dialogView";
import {
    Divider,
    TextField,
    Button,
...
export default function App() {
    ...
    const handleSaveView = async (myView: any) => {
        ...
        if (!originalView) {
            try {
                myView.userAdded = "sdubos"; // Add userAdded field
    ...
    // Update debouncedQuery after a delay
    useEffect(() => {
            ...
            <div className="flex flex-1 h-[calc(100%-65px)]">
                ...
                <div className="w-1/4 flex flex-col gap-4 p-4 text-black overflow-y-auto h-full">
                    ...
                    <div className="bg-gray-100 flex-2  rounded-lg shadow-md overflow-y-auto max-h-[calc(80vh-10rem)]">
                        <h1 className="text-lg font-bold text-gray-800 border-b pb-2 mb-3 p-4">
                            Data Lakehouse
                        </h1>
                        <MyTreeViewModule selectedTreeViewItem={selectedItem?.name || ""} onItemSelected={handleTreeItemSelected} />
                    </div>
                </div>
            </div>
        </div>
